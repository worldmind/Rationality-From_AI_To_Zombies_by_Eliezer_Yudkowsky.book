...
Для начала работы над переводом, перейдите по [ссылке](https://github.dev/worldmind/Rationality-From_AI_To_Zombies_by_Eliezer_Yudkowsky.book/tree/main/book.ru/), в папке book.ru находится копия книги, в которой текст на английском нужно заменить на русский текст.
...
Pull request можно создавать и раньше, достаточно сделать один коммит с каким-нибудь изменением, это позволит по ходу работы оставлять комментарии в запросе на вытягивание (как общие так и для конкретных строк).
Идеально если все вопросы и замечания сохранены как комментарии к соотвествующей строке.
...
После того как вы проверили измененные файлы, завершите ваше ревью. Для этого нажмите книпку "Review Changes", выберете вердикт (например Approve если всё хорошо), впишите общий для вашего ревью комментарий и нажмите кнопку `Submit Review`.
